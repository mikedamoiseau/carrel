...
- MOBI/AZW parsing uses libmobi (C) via `unsafe` FFI on untrusted input; the from-source builds (Windows + arm64-macOS release) pin `LIBMOBI_VERSION` to a full commit SHA (`85dcfe8`, the commit tag v0.12 points at) rather than to the tag itself, so a retargeted upstream tag cannot silently change what gets built — ci.yml and release.yml must carry the same SHA, drift-enforced by `src-tauri/src/release_workflow_test.rs`, while package-manager builds (Linux/macOS CI, local dev) track the distro version — see the security note atop `carrel-core/src/mobi/mod.rs` for the trust boundary and bump process
