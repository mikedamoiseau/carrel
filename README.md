...
- Table of contents panel (EPUB/MOBI), bookmarks in every format, text highlighting with notes in 5 colors (EPUB/MOBI), and dictionary lookups ("Define") on a text selection, with one-tap saving to the vocabulary builder, a book-scoped saved-words drawer, and a cross-book Vocabulary screen with search, sort, and delete over every saved word (EPUB/MOBI) — all shared with the desktop app
