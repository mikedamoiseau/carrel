...
///
/// The `carrel-backup-…` service prefix used here and in [`load_secrets`] /
/// [`remove_secrets`] keys entries in the OS keychain — changing it orphans
/// stored credentials rather than migrating them.
pub fn store_secrets(config: &BackupConfig) -> CarrelResult<BackupConfig> {
    ...
    for key in &secrets {
        if let Some(value) = config.values.get(*key) {
            ...
            let service = format!("carrel-backup-{:?}-{}", config.provider_type, key);
            let entry = keyring::Entry::new(&service, "default").map_err(|e| match e {
                keyring::Error::PlatformFailure(err) => {
                    CarrelError::internal(format!("Keychain unavailable for {key}: {err}"))
                }
                _ => CarrelError::internal(format!("Failed to access keychain for {key}: {e}")),
            })?;
            entry.set_password(value).map_err(|e| match e {
                ...
                _ => CarrelError::internal(format!(
                    "Failed to store secret '{key}' in keychain: {e}"
                )),
            })?;
...
pub fn load_secrets(config: &mut BackupConfig) -> CarrelResult<()> {
    let secrets = secret_keys(&config.provider_type);
    for key in &secrets {
        ...
        let entry = keyring::Entry::new(&service, "default").map_err(|e| {
            CarrelError::internal(format!("Failed to access keychain for {key}: {e}"))
        })?;
        let pw = entry.get_password().map_err(|e| match e {
            keyring::Error::NoEntry => {
                CarrelError::not_found(format!("Secret '{key}' not found in keychain"))
            }
            _ => CarrelError::internal(format!("Failed to load secret '{key}' from keychain: {e}")),
        })?;
...
pub fn remove_secrets(config: &BackupConfig) -> CarrelResult<()> {
    let secrets = secret_keys(&config.provider_type);
    for key in &secrets {
        let service = format!("carrel-backup-{:?}-{}", config.provider_type, key);
...
pub fn build_operator(config: &BackupConfig) -> CarrelResult<Operator> {
    ...
    fn make_blocking(async_op: opendal::Operator) -> CarrelResult<Operator> {
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            let _guard = handle.enter();
            Operator::new(async_op).map_err(|e| {
                CarrelError::internal(format!("Failed to create blocking operator: {e}"))
            })
        } else {
    ...
    match config.provider_type {
        ProviderType::S3 => {
            ...
            let async_op = opendal::Operator::new(builder)
                .map(|b| b.finish())
                .map_err(|e| CarrelError::network(format!("Failed to create S3 operator: {e}")))?;
        ...
        ProviderType::Ftp => {
            ...
            let async_op = opendal::Operator::new(builder)
                .map(|b| b.finish())
                .map_err(|e| CarrelError::network(format!("Failed to create FTP operator: {e}")))?;
            make_blocking(async_op)
        }
        #[cfg(feature = "sftp")]
        ProviderType::Sftp => {
            ...
            let async_op = opendal::Operator::new(builder)
                .map(|b| b.finish())
                .map_err(|e| {
                    CarrelError::network(format!("Failed to create SFTP operator: {e}"))
                })?;
            make_blocking(async_op)
        }
        #[cfg(not(feature = "sftp"))]
        ProviderType::Sftp => Err(CarrelError::invalid(
        ...
        ProviderType::Webdav => {
            ...
            let async_op = opendal::Operator::new(builder)
                .map(|b| b.finish())
                .map_err(|e| {
                    CarrelError::network(format!("Failed to create WebDAV operator: {e}"))
                })?;
            make_blocking(async_op)
        }
        ProviderType::Fs => {
            ...
            let async_op = opendal::Operator::new(builder)
                .map(|b| b.finish())
                .map_err(|e| CarrelError::io(format!("Failed to create FS operator: {e}")))?;
            make_blocking(async_op)
        }
    }
}
...
pub fn write_manifest(op: &Operator, manifest: &SyncManifest) -> CarrelResult<()> {
    let json = serde_json::to_string_pretty(manifest)?;
    op.write("manifest.json", json.into_bytes())
        .map(|_| ())
        .map_err(|e| CarrelError::network(format!("Failed to write manifest: {e}")))
}

pub fn push_json(op: &Operator, path: &str, data: &impl Serialize) -> CarrelResult<()> {
    let json = serde_json::to_string(data)?;
    op.write(path, json.into_bytes())
        .map(|_| ())
        .map_err(|e| CarrelError::network(format!("Failed to write {path}: {e}")))
}

pub fn pull_json<T: serde::de::DeserializeOwned>(op: &Operator, path: &str) -> CarrelResult<T> {
    let data = op
        .read(path)
        .map_err(|e| CarrelError::network(format!("Failed to read {path}: {e}")))?;
...
pub fn push_file_if_missing(
    ...
    let local_size = std::fs::metadata(local_path)
        .map(|m| m.len())
        .map_err(|e| CarrelError::io(format!("Cannot read {local_path}: {e}")))?;
    ...
    op.write(remote_path, data)
        .map_err(|e| CarrelError::network(format!("Failed to upload {remote_path}: {e}")))?;
    Ok(true)
}
...
mod tests {
    ...
    impl crate::storage::Storage for MockStorage {
        fn put(&self, _key: &str, _bytes: &[u8]) -> CarrelResult<()> {
        ...
        fn get(&self, key: &str) -> CarrelResult<Vec<u8>> {
            ...
            self.data
                ...
                .get(key)
                .cloned()
                .ok_or_else(|| CarrelError::not_found(format!("mock missing {key}")))
        }
        fn exists(&self, key: &str) -> CarrelResult<bool> {
            Ok(self.data.lock().unwrap().contains_key(key))
        }
        fn delete(&self, _key: &str) -> CarrelResult<()> {
            unimplemented!("MockStorage does not support delete")
        }
        fn list(&self, _prefix: &str) -> CarrelResult<Vec<String>> {
        ...
        fn local_path(&self, _key: &str) -> CarrelResult<std::path::PathBuf> {
            Err(CarrelError::invalid(
    ...
    fn test_connection_ok_with_fs_provider() {
        ...
        let probe_dir = dir.path().join(".carrel-connection-test");
