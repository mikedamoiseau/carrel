...
fn open_archive(path: &str) -> CarrelResult<ZipArchive<std::fs::File>> {
    let file =
        std::fs::File::open(path).map_err(|e| CarrelError::io(format!("Cannot open file: {e}")))?;
    let mut archive = ZipArchive::new(file)
        .map_err(|e| CarrelError::invalid(format!("Not a valid ZIP/CBZ archive: {e}")))?;
...
fn read_page_bytes(archive: &mut ZipArchive<std::fs::File>, name: &str) -> CarrelResult<Vec<u8>> {
    let entry = archive
        .by_name(name)
        .map_err(|e| CarrelError::not_found(format!("Cannot read page '{name}': {e}")))?;
...
pub fn import_cbz(path: &str) -> CarrelResult<CbzMeta> {
...
mod tests {
    ...
    fn import_cbz_rejects_comicinfo_lying_about_decompressed_size() {
        ...
        assert!(
            matches!(err, CarrelError::InvalidInput(_)),
    ...
    fn page_reads_reject_entry_lying_about_decompressed_size() {
        ...
        assert!(
            matches!(bytes_err, CarrelError::InvalidInput(_)),
        ...
        assert!(
            matches!(uri_err, CarrelError::InvalidInput(_)),
