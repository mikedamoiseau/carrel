...
//!
//! Because of that exposure the libmobi version is pinned wherever Carrel builds
...
impl MobiBook {
    ...
    /// Carrel that's fine because imports are copied into the app-managed
    /// library directory (`~/Documents/Carrel Library/...`) where the filenames are
    ...
    pub fn open(path: &Path) -> CarrelResult<Self> {
        let path_str = path.to_str().ok_or_else(|| {
            CarrelError::InvalidInput(format!("MOBI path is not valid UTF-8: {}", path.display()))
        })?;
        let c_path = CString::new(path_str.as_bytes())
            .map_err(|_| CarrelError::InvalidInput("MOBI path contained a NUL byte".into()))?;
        ...
        unsafe {
            ...
            if handle.is_null() {
                return Err(CarrelError::Internal(
    ...
    /// outlive it.
    pub fn rawml(&self) -> CarrelResult<MobiRawml<'_>> {
        ...
        let ptr = NonNull::new(ptr)
            .ok_or_else(|| CarrelError::Internal("libmobi failed to allocate rawml".into()))?;
...
/// Translate a libmobi `MOBI_RET` code to a [`CarrelError`] preserving the
/// semantic category. Keeps the raw code in the message so the original
/// libmobi failure is still visible in logs.
fn map_mobi_ret(rc: ffi::MOBI_RET, context: &str) -> CarrelError {
    let msg = format!("{context}: libmobi code {rc}");
    match rc {
        ffi::MOBI_RET_MOBI_FILE_NOT_FOUND => CarrelError::NotFound(msg),
        ...
        | ffi::MOBI_RET_MOBI_DRM_EXPIRED => CarrelError::InvalidInput(msg),
        ...
        | ffi::MOBI_RET_MOBI_ERROR => CarrelError::Internal(msg),
        _ => CarrelError::Internal(msg),
    }
}
...
mod tests {
    ...
    fn fixture(name: &str) -> Option<PathBuf> {
        ...
        // (gitignored). carrel-core is built from the workspace root, so
        // CARGO_MANIFEST_DIR points to carrel-core/.
