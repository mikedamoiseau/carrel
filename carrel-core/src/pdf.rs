...
fn bind_pdfium() -> CarrelResult<PdfiumSession> {
    ...
    let bindings = match PDFIUM_LIBRARY_PATH.get().and_then(|p| p.as_deref()) {
        Some(path) => {
            let path_str = path
                .to_str()
                .ok_or_else(|| CarrelError::internal("pdfium path is not valid UTF-8"))?;
...
pub fn import_pdf(path: &str) -> CarrelResult<PdfMeta> {
    let pdfium = bind_pdfium()?;
    let document = pdfium
        .load_pdf_from_file(path, None)
        .map_err(|e| CarrelError::invalid(format!("failed to open PDF: {e}")))?;
...
pub fn get_page_image(path: &str, page_index: u32, width: u32) -> CarrelResult<String> {
    ...
    let bitmap = page
        ...
        .map_err(|e| CarrelError::internal(format!("render failed: {e}")))?;

    let img = bitmap.as_image();
    let mut jpeg_bytes: Vec<u8> = Vec::new();
    let encoder = image::codecs::jpeg::JpegEncoder::new_with_quality(&mut jpeg_bytes, 90);
    img.write_with_encoder(encoder)
        .map_err(|e| CarrelError::internal(format!("JPEG encode failed: {e}")))?;
...
) -> CarrelResult<(Vec<u8>, &'static str)> {
    let pdfium = bind_pdfium()?;
    let document = pdfium
        .load_pdf_from_file(path, None)
        .map_err(|e| CarrelError::invalid(format!("failed to open PDF: {e}")))?;
...
fn extract_all_page_texts(path: &str) -> CarrelResult<Vec<String>> {
    let pdfium = bind_pdfium()?;
    let document = pdfium
        .load_pdf_from_file(path, None)
        .map_err(|e| CarrelError::invalid(format!("failed to open PDF: {e}")))?;
    ...
    for page_idx in 0..page_count {
        let page = pages
            .get(page_idx)
            .map_err(|e| CarrelError::not_found(format!("page {page_idx} not found: {e}")))?;
        let text = page.text().map_err(|e| {
            CarrelError::internal(format!("failed to extract text from page {page_idx}: {e}"))
        })?;
...
fn peek_cached_page_texts(path: &str) -> CarrelResult<Option<Vec<String>>> {
...
fn store_cached_page_texts(path: &str, texts: &[String]) -> CarrelResult<()> {
...
fn get_cached_page_texts(path: &str) -> CarrelResult<Vec<String>> {
...
where
    F: Fn(&str) -> CarrelResult<Vec<String>>,
{
    if let Some(texts) = peek_cached_page_texts(path)? {
        return Ok(texts);
    }
...
pub fn search_pdf(path: &str, query: &str) -> CarrelResult<Vec<PdfSearchResult>> {
...
pub fn page_texts_memory(path: &str) -> CarrelResult<Vec<String>> {
...
pub fn cached_page_text(path: &str, page_index: usize) -> CarrelResult<Option<String>> {
    let cache = PDF_TEXT_CACHE.lock()?;
    Ok(cache
        .get(path)
...
pub fn search_pdf_with_storage(
    ...
    storage: &dyn Storage,
    book_hash: &str,
) -> CarrelResult<Vec<PdfSearchResult>> {
...
fn glyph_cache_get(path: &str, page_index: usize) -> CarrelResult<(Option<Vec<Glyph>>, u64)> {
...
pub fn get_page_glyphs(path: &str, page_index: usize) -> CarrelResult<Vec<Glyph>> {
    ...
    .ok_or_else(|| CarrelError::invalid(format!("page {page_index} has invalid dimensions")))?;
...
mod tests {
    ...
    fn test_search_uses_disk_index_without_extracting() {
        ...
        let extractor = |_path: &str| -> CarrelResult<Vec<String>> {
            extractor_calls.fetch_add(1, Ordering::SeqCst);
            Err(CarrelError::internal("extractor should not be called"))
        };
