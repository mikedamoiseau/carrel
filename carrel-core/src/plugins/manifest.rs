...
pub fn parse_manifest(raw: &str) -> CarrelResult<PluginManifest> {
    let raw: RawManifest = toml::from_str(raw)
        .map_err(|e| CarrelError::invalid(format!("plugin.toml parse error: {e}")))?;
    ...
    for event in &raw.events.subscribe {
        if !CarrelEvent::ALL_NAMES.contains(&event.as_str()) {
            return Err(CarrelError::invalid(format!(
    ...
    for p in &raw.permissions.required {
        let parsed = Permission::parse(p)
            .ok_or_else(|| CarrelError::invalid(format!("unknown permission '{p}'")))?;
