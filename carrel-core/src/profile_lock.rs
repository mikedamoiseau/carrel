...
/// web PIN).
const KEYRING_SERVICE_PREFIX: &str = "com.mike.carrel.profile-lock";
...
fn entry(profile: &str) -> CarrelResult<keyring::Entry> {
    keyring::Entry::new(&keyring_key(profile), KEYRING_USER)
        .map_err(|e| CarrelError::internal(format!("Failed to access keychain: {e}")))
}

/// Argon2id-hash `password` into a PHC string suitable for storage.
pub fn hash_password(password: &SecretString) -> CarrelResult<String> {
    let salt = SaltString::generate(&mut OsRng);
    argon2()
        .hash_password(password.expose_secret().as_bytes(), &salt)
        .map(|hash| hash.to_string())
        .map_err(|e| CarrelError::internal(format!("Failed to hash password: {e}")))
}
...
fn validate_policy(parsed: &PasswordHash) -> CarrelResult<()> {
    let out_of_policy =
        |what: &str| CarrelError::invalid(format!("Out-of-policy password hash: {what}"));
...
pub fn verify_password(password: &SecretString, phc: &str) -> CarrelResult<bool> {
    let parsed = PasswordHash::new(phc)
        .map_err(|e| CarrelError::invalid(format!("Malformed password hash: {e}")))?;
...
fn map_load_result(result: Result<String, keyring::Error>) -> CarrelResult<Option<String>> {
    match result {
        Ok(phc) => Ok(Some(phc)),
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(e) => Err(CarrelError::internal(format!("Keychain error: {e}"))),
    }
}

/// Read the stored PHC string for `profile`. `Ok(None)` only on "no lock
/// set" (`NoEntry`); any other keychain failure is `Err` (Decision 7).
pub fn load_lock(profile: &str) -> CarrelResult<Option<String>> {
...
pub fn set_lock(profile: &str, phc: &str) -> CarrelResult<()> {
    let parsed = PasswordHash::new(phc)
        .map_err(|e| CarrelError::invalid(format!("Malformed password hash: {e}")))?;
    validate_policy(&parsed)?;
    entry(profile)?
        .set_password(phc)
        .map_err(|e| CarrelError::internal(format!("Failed to store profile lock: {e}")))
}

/// Remove the lock for `profile`, if any. Ignores `NoEntry` — clearing an
/// already-unlocked profile is not an error.
pub fn clear_lock(profile: &str) -> CarrelResult<()> {
...
pub fn has_lock(profile: &str) -> CarrelResult<bool> {
