...
fn validated_file_name(hash: &str, ext: &str) -> CarrelResult<String> {
...
pub fn run_eviction(base: &Path, max_size_mb: u64) -> CarrelResult<()> {
    let dir = base.join(SOURCE_CACHE_DIR);
...
pub fn stage(base: &Path, src: &Path, hash: &str, ext: &str) -> CarrelResult<PathBuf> {
    ...
    let dir = dest
        .parent()
        .ok_or_else(|| CarrelError::internal("source-cache path has no parent"))?;
    ...
    let publish = || -> CarrelResult<()> {
        ...
        if copied != src_len {
            return Err(CarrelError::io(format!(
