...
**Web reader:** the phone/web reader offers the same lookup for EPUB and MOBI
(see *Remote Access* below) — it needs a connection to your Carrel server, so
it isn't available while reading offline-saved books.

> Notes: the dictionary is English and offline-only in this version (an online
> fallback and custom dictionaries may come later). In the web reader, Define
> is available for EPUB and MOBI only — page-image formats (PDF, CBZ, CBR)
> show only page images there, with no selectable text to define from.
...
**Web reader:** the phone/web reader has a bookmark button (🔖) in the reader toolbar, in every format. Tap it to open a slide-in panel, then **Add bookmark here** to save your current spot; each bookmark shows its chapter (or page) and how far through it is. Tap a bookmark to jump straight back to it, use the ✏️ to give it a name (adding a bookmark drops you straight into naming it), or use the ✕ to delete one. Bookmarks sync with the desktop app and your other devices.
...
- **Define a word (web)** — while reading an EPUB or MOBI, select some text to see the same highlight popover as before, now with a **Define** button (only when you've installed and enabled the dictionary — see *Dictionary / word lookup* in section 4). It looks up the selection — or, for a selection longer than three words (or 100 characters), just its first word — and shows the matched word's definitions by part of speech in a small card, or says so if the word isn't found. It needs a connection to your Carrel server, so it isn't offered while reading an offline-saved book. When you've also turned on **Build my vocabulary list** (see *Vocabulary builder* below), a matched definition's card gains a **Save** button that logs the word to your vocabulary list; it isn't offered otherwise, and saving it flips the button to a disabled "Saved" state rather than showing a toast.
